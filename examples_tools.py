...
def example_1_basic_math_tools():
    ...
    llm.add_tool_from_function(calculate_percentage, description="Calculate percentage of a value")
...
def example_2_web_api_tools():
    ...
    # Example conversation
    try:
        response = llm.prompt("What time is it now? Also, can you search for information about 'machine learning' in the tech category?")
        print(f"LLM Response: {response}")
    except Exception as e:
        print(f"Error: {e}")
...
def example_3_advanced_tool_chain():
    ...
    llm.add_tool_from_function(filter_data, description="Filter JSON data by field value")
    llm.add_tool_from_function(calculate_average, description="Calculate average of numeric field")
    llm.add_tool_from_function(generate_report, description="Generate formatted report from data")
...
def example_4_tool_management():
    ...
    llm.add_tool_from_function(greeting_tool, description="Generate multilingual greetings")
...
def main():
    ...
    try:
        ...
        print("Note: Make sure to set up valid API credentials in environment variables.")
