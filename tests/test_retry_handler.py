...
class TestRetryHandler:
    ...
    def test_execute_no_retry_on_403(self):
        ...
        error = APIStatusError("Forbidden", response=mock_response, body={"error": "forbidden"})
    ...
    def test_log_403_cloudfront_detection(self):
        ...
        with patch('floship_llm.retry_handler.logger') as mock_logger:
            ...
            assert any('CloudFront' in str(call) for call in mock_logger.error.call_args_list)
    ...
    def test_retryable_status_codes(self):
        ...
        for status_code in retryable_codes:
            ...
            mock_func = Mock(side_effect=[
                APIStatusError(f"Error {status_code}", response=mock_response, body={}),
                "success"
            ])
    ...
    def test_non_retryable_status_codes(self):
        ...
        for status_code in non_retryable_codes:
            ...
            error = APIStatusError(f"Error {status_code}", response=mock_response, body={})
            mock_func = Mock(side_effect=error)
