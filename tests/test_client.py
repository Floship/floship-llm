...
class TestLLM:
    ...
    def test_init_missing_env_vars(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError, match="INFERENCE_URL environment variable must be set"):
                LLM()
                
        with patch.dict(os.environ, {'INFERENCE_URL': 'test'}, clear=True):
            with pytest.raises(ValueError, match="INFERENCE_MODEL_ID environment variable must be set"):
                LLM()
                
        with patch.dict(os.environ, {'INFERENCE_URL': 'test', 'INFERENCE_MODEL_ID': 'test'}, clear=True):
            with pytest.raises(ValueError, match="INFERENCE_KEY environment variable must be set"):
                LLM()

    def test_init_invalid_type(self):
        """Test LLM initialization with invalid type."""
        with patch('floship_llm.client.OpenAI'):
            with pytest.raises(ValueError, match="type must be 'completion' or 'embedding'"):
                LLM(type='invalid')
    ...
    def test_init_invalid_response_format(self):
        ...
        with patch('floship_llm.client.OpenAI'):
            with pytest.raises(ValueError, match="response_format must be a subclass of BaseModel"):
    ...
    def test_prompt_with_response_format(self):
        ...
        with patch('floship_llm.client.OpenAI') as mock_openai:
            ...
            with patch('floship_llm.utils.lm_json_utils.extract_strict_json') as mock_extract:
    ...
    def test_process_response_with_response_tags(self):
        ...
        with patch('floship_llm.client.OpenAI'):
            ...
            mock_choice = Mock()
            mock_choice.message.content = "Some text<response>Real response</response>More text"
            mock_response = Mock()
            mock_response.choices = [mock_choice]
    ...
    def test_process_response_max_length_exceeded(self):
        ...
        with patch('floship_llm.client.OpenAI'):
            ...
            mock_choice = Mock()
            mock_choice.message.content = "This is a very long response that exceeds the limit"
            mock_response = Mock()
            mock_response.choices = [mock_choice]
            
            with patch.object(llm, 'retry_prompt', return_value="retry_response") as mock_retry:
                result = llm.process_response(mock_response)
