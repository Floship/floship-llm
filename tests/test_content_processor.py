...
class TestContentProcessor:
    ...
    def test_truncate_content_above_limit(self):
        ...
        assert (
            "[Content truncated]" in result or "..." in result or result.endswith("...")
        )
    ...
    def test_process_tool_response_with_metadata(self):
        ...
        result = processor.process_tool_response(
            content, "test_tool", execution_time=0.5
        )
    ...
    def test_sanitize_tool_content_null_like(self):
        ...
        for null_str in ["None", "null", "NULL", "Null"]:
            result = processor.sanitize_tool_content(
                null_str, "test_tool", is_error=False
            )
