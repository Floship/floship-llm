...
class ContentProcessor:
    ...
    def __init__(
        ...
        self.sanitization_patterns = sanitization_patterns or self.DEFAULT_SANITIZATION_PATTERNS
    ...
    def sanitize_tool_content(
        ...
        """
        if content is None:
            default_msg = (f"Error executing tool: {tool_name}" if is_error 
        ...
        if not content_str:
            default_msg = (f"Error executing tool: {tool_name} (empty result)" if is_error
        ...
        if content_str.lower() in ["none", "null", "undefined"]:
            default_msg = (f"Error executing tool: {tool_name} (null result)" if is_error
